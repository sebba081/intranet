...
/**
 * Tabla genérica con filtro y paginación.
 * Acepta cualquier objeto plano: las columnas se derivan de sus claves.
 */
export function DataTable<T extends object>({ title, rows }: { title: string; rows: readonly T[] }) {
              ...
              <tbody>
                {current.map((row, idx) => (
                  <tr className="border-b border-border" key={idx}>
                    {Object.values(row).map((value, cidx) => <td className="px-2 py-2" key={cidx}>{String(value ?? "")}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
